import { Class, Element, RuleType, AttackType, ConditionType, TargetType, SkillType, ActionPattern, RuleValueByType, TurnActionType, CounterAttackMode, ConditionHPStatus } from './Constants.js';
...
export class Battle {
    constructor(team, turns = 13) {
        ...
        this.hpStatus = ConditionHPStatus.fulfill;
    ...
    init() {
        ...
        for (const card of this.team.cards) {
            const hpUp = this.battleTurns[card.name].rules.filter((r) => r.type == RuleType.hpUp)
    ...
    startBattle() {
        for (let turn = 1; turn <= this.turns; turn++) {
            ...
            // Attack
            for (const card of this.team.getCardByActionOrder(turn)) {
                let attackType = AttackType.BasicAttack;
    ...
    beforeRound(attackType, cards) {
        ...
        for (const card of cards) {
            let preAttackRules = this.battleTurns[card.name].rules.filter((e) => e.isBeforeRoundRule());
        ...
        // 敵方
        const enemyPreAttackRules = this.enemyBattleTurn.rules.filter((e) => e.isBeforeRoundRule());
    ...
    startRoundPerCard(attackType, card) {
        ...
        // ---------------------------每個人攻擊前------------------------------------
        let preAttackRules = this.battleTurns[card.name].rules.filter((e) => e.isPreAttackRule());
        for (const rule of preAttackRules) {
            this.addRuleToTargets(rule, card, attackType);
        }
        ...
        if (attackType == AttackType.Guard) {
            ...
            const postGuardRules = this.battleTurns[card.name].rules
                .filter((r) => r.isPostGuardRule() && r.isConditionsFulfilled(card, this, this.currentTurnAction, attackType));
        ...
        let attackRule = [].concat(card.attackRule);
    ...
    endRound(attackType, cards) {
        ...
        if (this.enemyCard != null) {
            ...
            if (this.enemyCard.remainHp > 0) {
                ...
                // Pre-attack
                let preAttackRules = this.enemyBattleTurn.rules.filter((r) => r.isPreAttackRule() && r.isConditionsFulfilled(enemy, this, TurnActionType.attack, AttackType.BasicAttack));
                for (const rule of preAttackRules) {
                ...
                // attack
                let attackRule = [].concat(enemy.attackRule);
        ...
        for (const card of cards) {
            ...
            // ---------------------------回合結束------------------------------------
            // 持續傷害
            const poisonRules = this.enemyBattleTurn.rules.filter((e) => e.type == RuleType.poisonAttackState && e.parentCardName == card.name);
            const poisonVal = poisonRules.reduce((sum, r) => sum + Util.getNumber(r.value) * r.getMaxCount(), 0);
            if (poisonVal != null && poisonVal > 0) {
                ...
                let outputVal = poisonVal > 0 ? Util.getFloat32(poisonVal) : Util.getFloat32(0);
                let enemyDamageVal = Util.getFloat32(outputVal);
            ...
            // 持續治療
            const contHealRules = this.battleTurns[card.name].rules.filter((e) => e.type == RuleType.continueHealState && e.parentCardName == card.name);
            const contHealVal = contHealRules.reduce((sum, e) => sum + Util.getNumber(e.value), 0);
            if (contHealVal != null && contHealVal > 0) {
                const cardRules = this.battleTurns[card.name].rules
                    .filter((r) => r.isConditionsFulfilled(card, this, this.currentTurnAction, attackType));
                const ourBuffs = cardRules.filter((r) => r.type == RuleType.partyContinueHealUp || r.type == RuleType.partyAllHealUp);
                const buffs = {};
                ...
                let outputVal = Util.getFloat32(contHealVal);
    ...
    processRule(rule, card, attackType) {
        ...
        for (const targetName of targetNames) {
            ...
            // 設定階段
                ...
                const phases = rule.value || rule.value;
    ...
    addRuleToTargets(rule, card, attackType) {
        ...
        for (const targetName of targetNames) {
            if (rule.type == RuleType.appendRule) {
                const newRule = rule.value.cloneSimple();
                ...
                let ruleName = rule.value;
                let clearCount = (_a = rule.maxCount) !== null && _a !== void 0 ? _a : 99;
    ...
    attack(attackType, rule, card) {
        ...
        if (rule.valueBy == RuleValueByType.exactVal) {
            ...
            let atkVal = Util.getFloat32(atk).multiply(Util.getFloat32(buffs[RuleType.atkUp]).add(1)).floor();
        ...
        let enemyDamageVal = new Float32(outputVal.getValue());
        for (const key of Object.keys(debuffs)) {
    ...
    getRuleTypesForOutput(card, outputOption) {
        const ruleType = [];
    ...
    getTurnValue(cardname, turn, outputOption) {
        const card = this.team.getCard(cardname);
    ...
    getTurnRuleLog(cardname, turn) {
        const attackType = this.battleTurns[cardname].action[turn];
        let rules = this.battleTurns[cardname].ruleLog[turn];
        const ruleTypes = new Set(rules.map(e => e.type));
        const acceptTypes = new Set(Battle.OUTPUT_TYPES);
...
export class BattleTurn {
    ...
    clearRuleByName(uniqueName, toClearCount = 99) {
    ...
    addRuleLog(turn, rule, applyCount = 1, extraInfo = null) {
        if (turn >= this.ruleLog.length) {
            return;
        }
        const existingRules = this.ruleLog[turn].filter(e => e.id == rule.id);
        const attackTypes = [RuleType.attack, RuleType.heal];
